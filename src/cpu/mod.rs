pub mod cpu {
    ...
    impl CpuStruct {
        ...
        pub fn interpret_command(&mut self) -> &mut CpuStruct {
            ...
            if !self.stopped {
                ...
                if self.instruction_register != 0xF3 {
                    //DI I think we effectively handle this twice.
        ...
        pub fn rst(&mut self) {
            ...
            self.cpu_state.change_r16(DoubleReg::SP, &|x| x - 2);
            self.cpu_state
                .set_pc((self.instruction_register & 0b00111000) as u16);
        }
        pub fn pop(&mut self) {
        ...
        pub fn add_sp_imm8(&mut self) {
            ...
            self.cpu_state.change_r16(registers::DoubleReg::SP, &|x| {
                x.wrapping_add_signed(operand as i16)
            });
        }
        ...
        pub fn bit(&mut self) {
            ...
            let val: u8 = self.cpu_state.get_r8_val(reg);
            self.cpu_state
                .set_flag(Flag::Zero, ((val >> bits) % 2) == 1);
            self.cpu_state.set_flag(Flag::Neg, false);
        ...
        pub fn stop(&mut self) {
            //No officialrom uses stop, so we're using the stop flag for halt instead
            self.stopped = true;
        }
